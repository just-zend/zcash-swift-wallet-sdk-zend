...
final class BlockEnhancerImplTests: XCTestCase {
    ...
    /// write is attempted once per enhance cycle (`retry` is cleared before the write, so the
    /// retry loop does not re-enter within the cycle); because a failed write leaves the data
    /// request pending in the backend, the next enhance cycle retries it naturally.
    func testThrowingSetTransactionStatusIsRetriedWithoutCrashing() async throws {
        ...
        XCTAssertEqual(
            ...
            1,
            "the status write is attempted once per enhance cycle; the pending request retries next cycle"
        )
    }
}
