...
public enum ZcashError: Equatable, Error {
    ...
    /// - `error` is the underlying claim-recording failure.
    /// ZRUST0124
    ...
    /// The process restarted between propose and confirm, a later proposal superseded the reviewed one, or the wallet changed underneath the cached plan. Propose again and re-confirm — ZIP 318 draws fresh schedule randomness on every proposal, so the SDK never silently signs a plan the user did not see.
    /// ZRUST0128
    ...
    /// - `rustError` contains error generated by the rust layer.
    /// ZRUST0139
    case rustMigrationDelivery(_ rustError: String)
    ...
    /// Every sampled server tip was below durable local wallet state. Retrying may select a healthy replica.
    /// ZCBPEO0024
    ...
    public var message: String {
        ...
        case .migrationRecordFailedAfterBroadcast: return "The migration engine failed to record a typed outcome under the exact submitted claim. The submit RPC did begin and the privacy sync gate is already marked; the claim remains resolution-only until Rust reconciles it from wallet chain evidence."
        ...
        case .rustMigrationDelivery: return "Error from the Rust-owned migration delivery runtime"
    ...
    public var code: ZcashErrorCode {
        ...
        case .rustMigrationDelivery: return .rustMigrationDelivery
