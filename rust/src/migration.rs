...
//!
//! Unlike the earlier prototype, the `zcash_pool_migration` crate derives no `serde`: its public
//! types expose `from_parts` constructors and accessor
...
fn zat_from_i64(amount: i64, what: &str) -> anyhow::Result<Zatoshis> {
    ...
    Zatoshis::from_u64(raw)
        .map_err(|e| anyhow!("{what} amount is not a valid zatoshi value: {e:?}"))
}
...
pub unsafe extern "C" fn zcashlc_migration_sign_note_split(
    ...
    let res = catch_panic(|| {
        ...
        let output_values = values
            ...
            .collect::<anyhow::Result<Vec<_>>>()?;
        let proposal =
            NoteSplitProposal::from_parts(output_values, zat_from_i64(fee, "note-split fee")?);
        let usk = unsafe { crate::decode_usk(usk_ptr, usk_len)? };
        let prepared = ctx.sign_note_split(&proposal, &usk).map_err(map_mig_err)?;
