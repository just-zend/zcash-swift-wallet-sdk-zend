...
for binary in "${binaries[@]}"; do
    ...
    for arch in "${archs[@]}"; do
        ...
        # __eh_frame still reference. That produces archives which llvm-nm accepts but both Apple
        # linkers reject. Remove only the embedded LLVM payload here; Apple strip below preserves
        # the relocation pairs while still bringing the universal archives under GitHub's limit.
        "$llvm_objcopy" \
