...
- `codex/zcash-upstream-sync-2026-06-17` merges upstream `main` through `fe836893`.
- The only merge conflict was `CHANGELOG.md`; resolve by keeping Zend's `2.6.3` release section and placing the newly merged upstream `Unreleased` diagnostics/resubmission notes above it.
...
- `adam/broadcaster-submit-plan`: 14 commits ahead and 68 behind `upstream/main`; no upstream PR, and the commits are already contained in `origin/main` from Zend PR `#2` before being superseded by upstream's `SubmitPlanStore` work in `#1757`.
- `adam/update-zcash-voting-0.9.1-policy`: 1 commit ahead and 64 behind `upstream/main`; voting feature scope with no upstream PR/review thread yet.
- `adam/voting-round-recovery-ffi`: 1 commit ahead and 173 behind `upstream/main`; voting recovery FFI with no upstream PR/review thread yet.
- `adam/voting-rust-lint-workflow`: 1 commit ahead and 174 behind `upstream/main`; workflow/lint only and limited direct Zend runtime value.
- `ignore_worktrees`: 1 commit ahead and 68 behind `upstream/main`; housekeeping-only change.
- `maint/v2.5.x`: 0 commits ahead and 154 behind `upstream/main`; maintenance line, no standalone carry.
- `release-ci`: 4 commits ahead and 146 behind `upstream/main`; release branch integration artifact, not a clear standalone carry target.
- `release/2.6.0`: 0 commits ahead and 157 behind `upstream/main`; upstream release stabilization branch, wait for upstream release sequencing.
- `roman/voting-delegation-workflow-swift-wrappers`: 0 commits ahead and 133 behind `upstream/main`; voting wrapper branch without an upstream PR/review thread yet.
- `shielded-vote-2.4.10`: 1 commit ahead and 91 behind `upstream/main`; specialized voting branch with unclear Zend product priority.
