...
The migration engine is private. A final release dependency must use the canonical
`ssh://git@github.com/just-zend/ZODLIronwoodMigrationRust.git` repository plus an immutable
40-character `rev` in `Cargo.toml`; never commit a developer's `path = ...` dependency. For an
authenticated release build, use a clean checkout at that exact revision:

```bash
export IRONWOOD_MIGRATION_ENGINE_REPO=/absolute/path/to/ZODLIronwoodMigrationRust
./Scripts/build-ironwood-ffi-artifact.sh
```

That command audits the dependency graph, builds exactly the arm64 iOS-device, iOS-simulator, and
macOS slices at the Swift package's iOS 13/macOS 12 deployment floor (the arm64 simulator slice is
iOS 14 because that simulator architecture did not exist on iOS 13), and writes:

- `BuildSupport/IRONWOOD_FFI_BUILD.env` — canonical migration repository, immutable revision/tree,
  source epoch, and Rust toolchain recipe;
- `LocalPackages/IRONWOOD_FFI_PROVENANCE.env` — recipe plus librustzcash revision, SDK source hash,
  deployment targets, Apple SDK/Xcode versions, and every slice hash;
...
symbol referenced by the production Swift welding in every slice, verifies source/toolchain/slice
hashes and Mach-O platform floors, and rejects private checkout paths or Rust source material in
`LocalPackages`. Public GitHub
Actions deliberately does not fetch the private repository: the manual packaging workflow verifies
and uploads only the already committed, provenance-locked XCFramework and refuses to overwrite an
existing release.
...
belong outside that boundary, with only run/revision/claim-token state crossing back into Rust.
