...
| SDK-owned immediate-run table | Dropped as authority. The upstream immediate proposal remains the transaction/economics source, while Rust creates the additive `immediate` delivery lane and reserves its exact inputs before any signing or network exposure. The same claim, policy, recovery and finality rules apply; Swift does not infer immediate state after broadcast or use a side row to mask canonical `Complete`. |
...
| `proposeImmediateMigrationIntent` then `commitMigrationIntents` | `submitImmediateMigration` for an SDK-held key, or `prepareImmediateMigrationForExternalSigning` then `submitExternallySignedImmediateMigration` for hardware signing. |
...
7. Strict legacy cutover, Rust-owned immediate pre-exposure reservation, durable claim leases,
   resolution-only unknown outcomes and immutable typed policy binding.
...
- Delivery revision/CAS, typed policy binding, strict cutover, immediate pre-exposure reservation,
  capability leases, unknown-outcome resolution and the 101-confirmation reservation horizon are
  additive crash-safety capabilities, not a competing migration engine. Keep their rationale and
  invariant tests adjacent to the implementation for later upstream review.
...
2. Remove `sdk_invalid_marks` and `sdk_immediate_runs` as runtime authorities. Wire SDK
   ...
   CAS/claim APIs; test strict cutover, pre-exposure immediate reservation, policy binding,
